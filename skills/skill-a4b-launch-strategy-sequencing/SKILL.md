---
name: skill-a4b-launch-strategy-sequencing
description: Supporta decisioni strategiche su SE, QUANDO e COSA lanciare — readiness assessment, sequencing prodotti, finestra ottimale di lancio, trade-off tra urgenza e preparazione, portafoglio prodotti. USA QUESTA SKILL ogni volta che l'utente sta valutando un lancio ma non è ancora certo del go, deve decidere l'ordine dei prodotti da lanciare, vuole capire se è pronto o se serve più prep. Attiva quando dice "conviene lanciare ora", "sono pronto a lanciare", "quale prodotto lancio prima", "sequencing", "sono in ritardo sul lancio", "devo posticipare", "readiness", "timing del lancio", "quale offerta lancio per prima", "strategia di lancio", "lancio ora o aspetto", "ordine dei lanci". Non aspettare che dica "launch strategy" — se il contesto implica decidere SE, QUANDO o in CHE ORDINE lanciare, usa questa skill. NON usare se la decisione è già presa e serve solo il piano operativo (usa skill-a4-execution). Si combina con F1 (halo strategy) per decidere quale offerta sia centrale.
---
...
Questa skill è la madre: vale per qualsiasi tipologia di lancio (webinar, evergreen VSL, verticale, membership). Le skill figlie A4a e A4b gestiscono le meccaniche specifiche di webinar e VSL evergreen.
...
|---|---|---|
| **Prezzo prodotto** | €[X] | Definito in A2 |
...
3. **Scarcity reale**: Qualsiasi meccanismo di urgenza usato deve avere una motivazione credibile e reale. Vedi A2 per i principi.
